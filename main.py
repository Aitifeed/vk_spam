...
from config import admin_id,host,user,name_db,password,token_bot
from db import Database
from threading import Thread
from spam import spam

group_api = vk_api.VkApi(token=token_bot)
			...
			if event.user_id == admin_id:
					...
					if len(groups['id'])==0:
						...
						for i in groups['id']:
							if groups['id'][i]['status']==False:
								...
								send_message(event.user_id,'id-'+str(i)+',группа-'+groups['id'][i]['name']+',статус активированна!')

						send_message(event.user_id,'Отправьте id группы,чтобы активировать/деактивировать её!',keyboard)				
										...
										try:
											...
											if id_group not in groups['id']:
												...
												if status==True:
													...
													send_message(event.user_id,'Группа - '+name_group+',успешно деактивированна!\nМожете дальше отправлять id,чтобы активировать/деактивировать группы или нажать "Главное меню",чтобы прекратить процедуру.')
				...
				if text=='Посмотреть данные спам аккаунтов':
					...
					if len(accounts['id'])==0:
						send_message(event.user_id,'У вас нет ни одного спам аккаунта!')
					else:
				...
				if text=='Удалить группу':
					...
					if len(groups['id'])==0:
						send_message(event.user_id,'У вас нет ни одной группы,чтобы удалить её.\nСначала загрузите группу!')
					else:
						for i in groups['id']:
							send_message(event.user_id,'id-'+str(i)+', '+groups['id'][i]['name'])

						send_message(event.user_id,'Отправь мне id группы из списка которую будем удалять.\nОтправлять нужно только цифры без других символов!',keyboard)
										...
										try:
											...
											if id_group not in groups['id']:
												...
												if len(data_group['id'])==0:
													send_message(event.user_id,'Группа - '+name_group+',успешно удалена!',keyboard)
													break
												else:
													send_message(event.user_id,'Группа - '+name_group+',успешно удалена!\nМожете дальше отправлять id,чтобы удалять группы или нажать "Главное меню",чтобы прекратить процедуру удаления.')
				...
				if text=='Посмотреть спам текст групп':
					...
					if len(groups['id'])==0:
						send_message(event.user_id,'У вас нет ни одной группы,чтобы посмотреть спам текст.\nСначала загрузите группу.')
					else:
						for i in groups['id']:
							send_message(event.user_id,f'id-'+str(i)+', '+groups['id'][i]['name'])

						send_message(event.user_id,'Отправь мне id группы из списка для которой будем смотреть спам текст.\nОтправлять нужно только цифры без других символов!',keyboard)
									...
									if text == 'start' or text == 'Главное меню':
										...
										try:
											...
											if id_group not in groups['id']:
												...
												name_group=groups['id'][id_group]['name']
												if spam_text==None:
													send_message(event.user_id,'Для группы '+name_group+' нет спам текста!\n\nВы можите дальше отправлять id группы или нажать кнопку "Главное меню",чтобы вернуться.' )
												else:
													send_message(event.user_id,'Cпам текст для группы - '+name_group+':\n\n'+spam_text)
				...
				if text== 'Добавить или обновить спам текст':
					...
					print(groups)
					if len(groups['id'])==0:
						send_message(event.user_id,'У вас нет ни одной группы,чтобы добавить/обновить спам текст.Сначала загрузите группу.')
					else:
						for i in groups['id']:
							send_message(event.user_id,'id-'+str(i)+', '+groups['id'][i]['name'])

						send_message(event.user_id,'Отправь мне id группы из списка для которой будем добавлять/обновлять спам текст.\nОтправлять нужно только цифры без других символов!',keyboard)
									...
									if event.text == 'start' or event.text == 'Главное меню':
										...
										try:
											...
											if id_group not in groups['id']:
												...
												name_group=groups['id'][id_group]['name']
												send_message(event.user_id,'Отправь спам текст для группы - '+name_group)
												for event in VkLongPoll(group_api).listen():
													if event.type == VkEventType.MESSAGE_NEW and event.to_me:
														if event.user_id == admin_id:
				...
				if text== 'Загрузить группу':
					...
					send_message(event.user_id,'Отправь мне токен группы.',keyboard)
				...
				if text=='Загрузить спам аккаунт':
					keyboard= VkKeyboard()
					keyboard.add_button('Главное меню',VkKeyboardColor.PRIMARY)
					send_message(event.user_id,'Отправь мне логин,пароль и токен аккаунта без пробелов в формате - log:логин,pass:пароль,token:токен.\n\nЧтобы прекратить процедуру нажмите "Главное меню".',keyboard)
								...
								if event.text == 'Главное меню' or event.text=='start':
									...
									try:
										...
										if 'invalid access_token' in str(error):
											...
											send_message(event.user_id,'Неправильный токен! Пожалуйста проверьте правльность токена и повторить процедуру!',keyboard)
										...
										if 'user is blocked' in str(error):
											...
											send_message(event.user_id,'Данный аккаунт заблокирован!',keyboard)
				...
				if text=='Проверить статус спам аккаунтов':
					...
					if len(accounts['id'])==0:
						send_message(event.user_id,'У вас нет ни одного спам аккаунта!Сначала загрузите их.')
					else:
						for id_acc in accounts['id']:
							try:
				...
				if text=='Удалить спам аккаунт':
					...
					if len(accounts['id'])==0:
						send_message(event.user_id,'У вас нет ни одного спам аккаунта,чтобы его удалить!')
					else:
						for id_acc in accounts['id']:
							send_message(event.user_id,
						...
						send_message(event.user_id,'Отправь id аккаунта,чтобы его удалить.Чтобы прекратить процедуру нажмите "Главное меню".',keyboard)
									...
									if event.text == 'Главное меню' or event.text=='start':
										...
										try:
											...
											if id_acc not in accounts['id']:
												send_message(event.user_id,'Аккаунтов под id-'+str(id_acc)+' нет!Внимательно смотрите на список id спам аккаунтов!')
											else:
												db.del_spam_account(id_acc)
												send_message(event.user_id,'Аккаунт под id-'+str(id_acc)+' успешно удален!')

										except:
											send_message(event.user_id,'Не могу найти такой id,возможно вы ошиблись.\nОтправьте коректный id!',keyboard)
				...
				if text=='Начать работу':
					...
					if len(groups['id'])==0:
						send_message(event.user_id,'Нет активированных груп!Пожалуйста активруйте группы и повторите процедуру!')
					else:
						for i in groups['id']:
		 					Thread(target=spam,args=(i,groups['id'][i]['token'],groups['id'][i]['group_id'])).start()
		 					
		 					send_message(event.user_id,'Работа рассылок в группе - '+groups['id'][i]['name']+' началась!')